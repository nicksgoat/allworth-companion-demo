...
// The one place a combined net-worth figure and its trajectory are allowed: a
// tapped-in detail sheet (DESIGN.md), on the brand's premium navy field. Carries
// the synthetic-data disclaimer inline so the compliance note travels with the
// number.
export function NetWorthDetail({
  ...
  const asOf = d.dataStatus?.isStale
    ? "stale data"
    : d.dataStatus?.asOf
      ? `as of ${d.dataStatus.asOf}`
      : null;
