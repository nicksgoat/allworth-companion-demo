...
const DEMO_BOND_LADDER: BondLadderResult = (() => {
  const base = [
    ['F12345678', 'Evergreen Family', 'AWF - Bond Ladder Municipal 5 year', 'California State', 'CA', 102000, 4.25, 3.81, 'AA+', '2028-06-01'],
    ['F12345678', 'Evergreen Family', 'AWF - Bond Ladder Municipal 5 year', 'Texas Water Authority', 'TX', 98000, 3.90, 3.67, 'AAA', '2029-03-15'],
    ['S88421003', 'Northstar Household', 'AWF - Bond Ladder Corporate 7 year', 'Microsoft Corp', 'WA', 126000, 4.10, 4.02, 'AAA', '2030-02-06'],
    ['S88421003', 'Northstar Household', 'AWF - Bond Ladder Corporate 7 year', 'JPMorgan Chase', 'NY', 118000, 4.55, 4.18, 'A+', '2031-05-12'],
    ['F77190241', 'Harbor Ridge Household', 'AWF - Bond Ladder Treasury 5 year', 'US Treasury', 'DC', 205000, 3.88, 3.74, 'AAA', '2028-11-30'],
    ['F77190241', 'Harbor Ridge Household', 'AWF - Bond Ladder Municipal 5 year', 'New York MTA', 'NY', 87000, 4.35, 4.06, 'A', '2029-09-01'],
    ['S44900128', 'Cedar Grove Family', 'AWF - Bond Ladder Corporate 7 year', 'Home Depot Inc', 'GA', 112000, 4.20, 3.98, 'A', '2032-04-15'],
    ['S44900128', 'Cedar Grove Family', 'AWF - Bond Ladder Municipal 5 year', 'Illinois Finance Authority', 'IL', 76000, 4.80, 4.42, 'A-', '2027-12-01'],
  ] as const;
  const bonds: BondLadderHolding[] = base.map((row, index) => ({
    account_number: row[0], account_name: row[1], strategy: row[2], symbol: '',
    cusip: `DEMO${String(index + 1).padStart(5, '0')}`, description: `${row[3]} bond`, issuer: row[3], sector: row[2].includes('Municipal') ? 'Municipal' : row[2].includes('Treasury') ? 'Government' : 'Corporate', state: row[4], quantity: Number(row[5]) / 100, price: 100, market_value: Number(row[5]), coupon: Number(row[6]), yield_to_worst: Number(row[7]), effective_duration: 4.1 + index * 0.25, maturity_date: row[9], call_date: index % 3 === 0 ? '2027-10-01' : null, callable: index % 3 === 0, fitch_rating: row[8], fitch_rating_previous: index === 7 ? 'A' : row[8], fitch_rating_effective_date: index === 7 ? '2026-07-22' : null, fitch_rating_previous_effective_date: null, is_downgraded: index === 7, annual_income: Number(row[5]) * Number(row[6]) / 100,
  }));
  const accountMap = new Map<string, BondLadderAccountSummary>();
  for (const bond of bonds) {
    const current = accountMap.get(bond.account_number) ?? { account_number: bond.account_number, account_name: bond.account_name, strategy: bond.strategy, bond_count: 0, total_market_value: 0 };
    current.bond_count += 1; current.total_market_value += bond.market_value; accountMap.set(bond.account_number, current);
  }
  return { total_accounts: accountMap.size, total_bonds: bonds.length, total_market_value: bonds.reduce((sum, bond) => sum + bond.market_value, 0), strategies: [...new Set(bonds.map((bond) => bond.strategy))], accounts: [...accountMap.values()], bonds, cache_age_seconds: 18, fetched_at: '2026-08-08T20:45:00Z' };
})();
...
export const fetchBondLadder = async (
  ...
  if (DEMO_MODE) {
    const bonds = strategy ? DEMO_BOND_LADDER.bonds.filter((bond) => bond.strategy === strategy) : DEMO_BOND_LADDER.bonds;
    return { ...DEMO_BOND_LADDER, total_bonds: bonds.length, total_market_value: bonds.reduce((sum, bond) => sum + bond.market_value, 0), bonds: [...bonds].sort((a, b) => String(sortBy === 'call_date' ? a.call_date : a.maturity_date).localeCompare(String(sortBy === 'call_date' ? b.call_date : b.maturity_date))) };
  }
...
export const refreshBondLadder = async (): Promise<{ total_accounts: number; total_bonds: number; fetched_at: string | null }> => {
  if (DEMO_MODE) return { total_accounts: DEMO_BOND_LADDER.total_accounts, total_bonds: DEMO_BOND_LADDER.total_bonds, fetched_at: new Date().toISOString() };
