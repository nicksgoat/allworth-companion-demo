...
export default function PlanningApp() {
  ...
  const [reports, setReports] = useState<Array<{ id: number; name: string }>>([]);
  const [warehouseId, setWarehouseId] = useState('');
  const [importing, setImporting] = useState(false);
  const [importError, setImportError] = useState('');
  const [mcInputs, setMcInputs] = useState<MonteCarloInputs | null>(null);
    ...
    <Box className="plan-shell">
      ...
      <Container maxWidth="xl" sx={{ py: 3, px: { xs: 1, sm: 2, md: 3 } }}>
        ...
        {!loading && selected && <>
          ...
          <Box sx={{ backgroundColor: colors.surfaceCard, borderRadius: 4, border: '1px solid rgba(23,61,103,0.05)', boxShadow: '0 2px 8px rgba(12,46,78,0.05)', overflow: 'hidden' }}>
