...
import { AccountRow, DisclaimerFooter, HairlineDivider, LearnedFactRow, SectionHeader } from "../components/Rows";
...
function BookScreen() {
  ...
  useEffect(() => {
    app.api.book("dana").then(setBook).catch(() => {});
  }, []);
...
function HouseholdRow({ household, onPress }: { household: Household; onPress: () => void }) {
  ...
  return (
    <Pressable onPress={onPress} style={({ pressed }) => [styles.householdRow, pressed && { opacity: 0.7 }]}>
      <View style={[styles.householdAvatar, { backgroundColor: highlight ? colors.allworthNavy : colors.inkFaint }]}>
        <Text style={[styles.householdInitials, { color: highlight ? "#fff" : colors.inkSecondary }]}>{initials}</Text>
      </View>
      <View style={{ flex: 1, gap: 3 }}>
        <Text style={[styles.householdName, highlight && { fontWeight: "600" }]}>{household.name}</Text>
        <View style={{ flexDirection: "row", gap: 8 }}>
          <Text style={styles.householdManaged}>{usd(household.managedAssets)} managed</Text>
          {household.heldAwayDetected > 0 ? (
            <Text style={styles.householdHeldAway}>{usd(household.heldAwayDetected)} held away</Text>
          ) : null}
        </View>
      </View>
...
function ClientDetailScreen({ route }: NativeStackScreenProps<AdvisorStackParams, "ClientDetail">) {
  ...
  useEffect(() => {
    app.api.brief("dana", household.clientId).then(setBrief).catch(() => {});
  }, [household.clientId]);
...
const styles = StyleSheet.create({
  ...
  householdAvatar: { width: 40, height: 40, borderRadius: 20, alignItems: "center", justifyContent: "center" },
  ...
  nudgeHeadline: { fontSize: 13, fontWeight: "600", color: colors.attention, fontVariant: ["tabular-nums"] },
});
