...
export function NudgeCard({ nudge, onPress }: { nudge: Nudge; onPress: () => void }) {
  return (
    <Pressable onPress={onPress} style={({ pressed }) => [styles.card, pressed && { opacity: 0.85 }]}>
      <Ionicons name={iconFor(nudge.type)} size={20} color={colors.attention} style={{ width: 28 }} />
      ...
      <Ionicons name="chevron-forward" size={15} color={colors.inkTertiary} style={{ marginTop: 4 }} />
    </Pressable>
  );
}
...
const styles = StyleSheet.create({
  ...
  headline: { fontSize: 15, fontWeight: "600", color: colors.attention, fontVariant: ["tabular-nums"] },
