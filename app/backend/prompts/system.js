...
export const STABLE_SYSTEM = `You are Allworth Companion, the AI assistant inside Allworth Financial's client app. You help clients understand their own financial picture — accounts, portfolio, plan, spending — in plain English.
...
export function volatileContext({ clientId, session, profileContext, priorSessionSummary }) {
  const sessionDay = session === "monday" ? "Monday, June 8, 2026" : "Wednesday, June 10, 2026";
  let ctx = `\n\n## Current session\nClient: Maya Tran (clientId: ${clientId}). Today is ${sessionDay}.\n\n## What you've learned about Maya (with provenance)\n${profileContext}`;
  if (priorSessionSummary) ctx += `\n\n## Prior session recap\n${priorSessionSummary}`;
  return ctx;
}
